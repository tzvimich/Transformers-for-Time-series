...
if __name__=="__main__":
    ...
    for attention_heads in [1,2,4]:
        for big_dim in [32, 64,128, 256]:
            print("\nBig dim is: ", big_dim)
            print("attention_heads: ", attention_heads)
            step = horizon
            run(X_train, y_train, X_test, y_test, lr=0.001, encoders=8, heads=attention_heads, dff=None ,window_length=horizon, d_model=int(big_dim*(SAMPLE_SIZE / step)), big_dim=big_dim, step=horizon, do_pe=True, add_class_token=False, connecting_tokens=False, norm=True)
