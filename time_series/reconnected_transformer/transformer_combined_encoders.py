...
if __name__=="__main__":
    ...
    for encoders in range(2, 40, 4):
            big_dim=64
            attention_heads=2
            print("\nBig dim is: ", big_dim)
            print("attention_heads: ", attention_heads)
            step = horizon
            run(X_train, y_train, X_test, y_test, lr=0.001, encoders=encoders, heads=attention_heads, dff=None ,window_length=horizon, d_model=int(big_dim*(SAMPLE_SIZE / horizon)), big_dim=big_dim, step=horizon, do_pe=True, add_class_token=False, connecting_tokens=True, norm=True)
