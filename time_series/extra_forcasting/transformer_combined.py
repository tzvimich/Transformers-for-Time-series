...
def run(X_train, y_train, X_test, y_test, lr=0.001, encoders=2, heads=2, dff=None, window_length=WINDOW_LENGTH, step=STEP, d_model=60, big_dim=64, do_pe=False, add_class_token=False, connecting_tokens=True, norm=True, extras=0):
    dff = 4 * d_model
    ...
    model = Transformer(encoders=encoders, d_model=d_model, big_dim=big_dim, token_input=window_length+extras, num_heads=heads, dff=dff, rate=0, connecting_tokens=connecting_tokens, norm=norm)
    ...
    predictions = predictions[:,:-extras]
    truth = np.array(truth)
    print(predictions.shape)
    print(truth.shape)
    print("Smape on test set is: ", smape_numpy(truth, predictions))

def load_data_separate(data_type, extras):
        ...
        for i in mat[column]:
            if isinstance(i, float) and not math.isnan(i):
                ...
                if len(X_maybe) == X_CYCLES*horizon and xing:
                    ying = True
                    xing = False
                if len(y_maybe) == Y_CYCLES*horizon+extras and ying:
                    X.append(X_maybe)
                    y.append(y_maybe)
                    X_maybe = []
                    y_maybe = []
    ...
    assert y_train.shape[1] == horizon + extras
...
if __name__=="__main__":
    ...
    for extras in [2, 4, 8, 12, 0]:
        X_train, y_train, X_test, y_test, horizon = load_data_separate("monthly", extras)
        ...
        step = horizon
        run(X_train, y_train, X_test, y_test, lr=0.001, encoders=8, heads=attention_heads, dff=None ,window_length=horizon, d_model=int(big_dim*(SAMPLE_SIZE / horizon)), big_dim=big_dim, step=horizon, do_pe=True, add_class_token=False, connecting_tokens=True, norm=True, extras=extras)
