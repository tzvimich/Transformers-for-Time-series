...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        if self.connecting_tokens:
            ...
            ffn_output = tf.reshape(ffn_output, (ffn_output.shape[0], -1, self.d_model))
        else:
...
class Transformer(tf.keras.Model):
    ...
    def call(self, x, training):
        ...
        x_big = self.biggen(x_reshaped)
        x_split = tf.reshape(x_big, (x_big.shape[0], -1, self.d_model))
        for encoder in self.encoders:
            # print("calling 1")
            x = encoder(x_split, training, None)
            # print("called 1")
        flat = self.flatten(x)
        # last = x[:,-1:,:]
        # print(last.shape)
...
def run(X, y, lr=0.001, encoders=2, heads=2, dff=None, window_length=WINDOW_LENGTH, step=STEP, d_model=60, big_dim=128, do_pe=False, add_class_token=False, connecting_tokens=True, norm=True):
    dff = 4 * d_model
...
if __name__=="__main__":
    ...
    for big_dim in [128, 256, 512, 1024]:
        print("\nBig dim is: ", big_dim)
        run(X, y, lr=learning_rate, encoders=4, heads=1, dff=None ,window_length=12, d_model=int(big_dim/16), big_dim= big_dim, step=12, do_pe=True, add_class_token=False, connecting_tokens=True, norm=True)
