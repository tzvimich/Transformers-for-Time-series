...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, big_dim, d_model, num_heads, token_input, dff, rate, connecting_tokens, norm):
        ...
        if connecting_tokens:
            self.ffn = point_wise_feed_forward_network(big_dim*5, dff)
        else:
...
def load_data_separate(data_type):
    ...
    for i in range(len(m)):
        ...
        x = x[np.logizal_not(np.is_nan(x))]
        x = x[-X_CYCLES*horizon:]
        if len(x) == X_CYCLES*horizon:
            X_test.append(x)
            exist.append(i)
    X_test = np.array(X_test)
    y_test = mat_test.to_numpy()[:,1:]
    y_test = y_test[exist]
...
if __name__=="__main__":
    ...
    for big_dim in [32, 64, 128, 256]:
        print("\nBig dim is: ", big_dim)
        step=12
        run(X, y, lr=learning_rate, encoders=4, heads=1, dff=None ,window_length=12, d_model=int(big_dim*(SAMPLE_SIZE / step)), big_dim=big_dim, step=step, do_pe=True, add_class_token=False, connecting_tokens=True, norm=True)
