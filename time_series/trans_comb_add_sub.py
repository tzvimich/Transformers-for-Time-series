...
class Transformer(tf.keras.Model):
    def __init__(self, encoders, d_model, token_input, num_heads, dff, rate, connecting_tokens, norm):
        ...
        self.denses = [Dense(1) for _ in range(int(encoders/2))]
        self.res_sub = [Dense(token_input) for _ in range(int(encoders/2))]
...
if __name__=="__main__":
    ...
    lst = [(2, 1, 12, 12, SAMPLE_SIZE*4, learning_rate, SAMPLE_SIZE, True, True),
           (4, 1, 12, 12, SAMPLE_SIZE*4, learning_rate, SAMPLE_SIZE, True, True),
           (6, 1, 12, 12, SAMPLE_SIZE*4, learning_rate, SAMPLE_SIZE, True, True),
           (8, 1, 12, 12, SAMPLE_SIZE*4, learning_rate, SAMPLE_SIZE, True, True),
           (10, 1, 12, 12, SAMPLE_SIZE*4, learning_rate, SAMPLE_SIZE, True, True)]

    for encoders in range(10, 40, 2):
        window_length = 12
        step = 12
        heads = 1
        connecting_tokens = True
        norm=True
        d_model = SAMPLE_SIZE
        ...
        run(X, y, lr=learning_rate, encoders=encoders, heads=1, dff=SAMPLE_SIZE*4 ,window_length=window_length, d_model=d_model, step=step, do_pe=True, add_class_token=False, connecting_tokens=connecting_tokens, norm=norm)
