...
class Transformer(tf.keras.Model):
    ...
    def call(self, x, training):
        x, ind = tf.split(x, [self.d_model, 1], axis=-1)
        ind = tf.reshape(ind, (None, ))
        x = self.biggen(x)
        out = None
        block_input = x
...
def run(X_train, y_train, X_test, y_test, lr, encoders, heads, dff=None, window_length=WINDOW_LENGTH, step=STEP, d_model=60, big_dim=64, do_pe=False, add_class_token=False, connecting_tokens=True, norm=True):
    dff = 4 * d_model
...
def load_data_separate(data_type):
    ...
    X_train = np.array(X)
    y_train = np.array(y)
    X_total = np.concatenate((X_train, y_train), axis=1)
    y_train = []
    X_train = []
    for row in X_total:
        for i in range(X_CYCLES+1):
            X_train.append(np.concatenate((row[:int((i)*horizon)], np.zeros(horizon), row[int((i+1)*horizon):], np.array([i]))))
            y_train.append(row[int((i)*horizon):int((i+1)*horizon)])
    ...
    X_test = np.concatenate((X_test, np.zeros((horizon, len(X_test))), np.array([X_CYCLES]*len(X_test)).T))
    y_test = np.array(mat_test.values[:,1:])
    y_test = y_test[exist]
    assert len(X_test) == len(y_test)
    assert len(X_train) == len(y_train)
    assert X_train.shape[1] == X_CYCLES*horizon + horizon + 1
    assert y_train.shape[1] == horizon
    assert X_test.shape[1] == X_CYCLES*horizon + horizon + 1
...
if __name__=="__main__":
    ...
    SAMPLE_SIZE = X_train.shape[1] - 1

    for attention_heads in [1,2,4,8]:
        for big_dim in [64,128, 256]:
            print("\nBig dim is: ", big_dim)
            print("attention_heads: ", attention_heads)
            step = horizon
            run(X_train, y_train, X_test, y_test, lr=0.001, encoders=8, heads=attention_heads, dff=None ,window_length=horizon, d_model=int(big_dim*(SAMPLE_SIZE / horizon)), big_dim=big_dim, step=horizon, do_pe=True, add_class_token=False, connecting_tokens=False, norm=True)
